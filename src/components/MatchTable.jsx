                ...
                <tbody>
                    {matches.map((match, matchIndex) => (
                        <tr key={matchIndex + 1}>
                            ...
                            {match.team2 !== 'Spielfrei' && match.team1 !== 'Spielfrei' && <td><strong>{match.team1} <input className='w-15' type="number" min="0" value={match.results[0]}
                                onChange={(e) => handleResultChange(matchIndex, 0, e.target.value)} /> : <input className='w-15' type="number" min="0" value={match.results[1]}
